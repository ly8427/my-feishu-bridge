...
**让 cli.py / bridge.py 找到凭证**：设置环境变量（或写进 systemd unit）：
```bash
export FEISHU_ENV_FILE=~/.secrets/feishu-bridge.env
```
cli.py 即使没设也会自动回退到 `~/.secrets/feishu-bridge.env`，但显式设置
最稳妥。
...
验证：
```bash
docker exec feishu-claude-agent claude --version       # Claude 引擎
docker exec feishu-claude-agent opencode --version     # OpenCode 引擎
```
...
```
生成的 `~/.local/share/opencode/auth.json` 会被 compose 自动挂载进容器。
若你的 auth 文件在别处，设 `OPENCODE_AUTH_FILE` 指向它。
...
├─ "Missing .env" → FEISHU_ENV_FILE 没设/路径错 → §2.3
...
├─ Claude: ANTHROPIC_* 没注入 → docker exec feishu-claude-agent env | grep ANTHROPIC
│   (bridge.py 通过 docker exec -e 注入；若空，检查 env 文件的 ANTHROPIC_BASE_URL/AUTH_TOKEN)
